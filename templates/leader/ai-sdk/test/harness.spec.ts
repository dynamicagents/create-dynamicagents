...
import {
  ...
  parentPlugins,
  subagentPlugins
} from "@/agents/leader/plugins";
import { LEADER_CONFIG } from "@/config";

/**
 * The leader's tool surface, pinned.
 *
 * The parent and the subagent install **different** plugin lists: the parent orchestrates and reviews with git, a
...
const parent = () =>
  createAgentRuntime({ config: LEADER_CONFIG, plugins: parentPlugins(host()) });
