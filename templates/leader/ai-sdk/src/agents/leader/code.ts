 ...
 *
 * A subagent soul is an identity, and `validateRecipe` refuses to lend one, so
 * it belongs to the agent that chose it rather than to a library.
 *
 * This type is not an option the parent weighs — it is the *only* way work gets
 * done here. The parent has no shell, no writer and
