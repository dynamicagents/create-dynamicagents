...
export function container(env: Env, workspaceName: () => string) {
  return workspaceContainer(env.LEADER_WORKSPACE, workspaceName);
}
...
export const parentPlugins = (host: PluginHost<Env>): AgentPlugin[] => {
  ...
  const workspace = () =>
    host.env.LEADER_WORKSPACE.get(
      host.env.LEADER_WORKSPACE.idFromName(
        workspaceName(host.callerKey(), active.get())
      )
    );
  ...
  return [
    ...
    repo({
      ...
      git: workspaceGit({
        binding: host.env.LEADER_WORKSPACE,
