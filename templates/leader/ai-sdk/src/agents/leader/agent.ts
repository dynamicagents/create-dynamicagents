...
/**
 * The leader, with subtasks on core's own tool loop.
 *
 * A delegating round agent: the loop, the durable Subtask rows and the subagent
 * execution are all `@dynamicagents/core/round`, and the model pair is core's
 * Workers AI default.
 *
 * What it adds is the container underneath — so the overrides below are all
 * lifecycle, not inference: a weekly reclaim sweep for workspaces
...
export class LeaderAgent extends RoundAgentBase<Env> {
   ...
   *
   * The body is in `@/workspace/lifecycle.ts` because either harness owes its
