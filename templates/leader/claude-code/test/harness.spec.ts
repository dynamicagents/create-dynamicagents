...
import type { LeaderWorkspaceDO } from "@/index";
...
import { parentPlugins, subagentPlugins } from "@/agents/leader/plugins";
import {
  sessionBrief,
  sessionFooter,
  settleDrain,
  ...
  writingNote,
  type LeaderSubagent
} from "@/agents/leader/subagent";
import { LEADER_CONFIG, MODEL } from "@/config";
import { CLAUDE_CODE_SESSION } from "@/harness";
import {
  claudeCodeConfig,
  noWorkspaceRouting,
  GH_TOKEN_PLACEHOLDER
} from "@/agents/leader/claude-code";
import { gitIdentity } from "@/workspace/git-identity";

/**
 * The leader's wiring, pinned.
 *
...
const host = (): PluginHost<Env> =>
  ({
    ...
    aiGatewayId: LEADER_CONFIG.model?.aiGatewayId ?? "default"
  }) as PluginHost<Env>;
...
describe("the parent's surface", () => {
  ...
  /**
   * `parentPlugins` is this harness's own list: a review tool dropped from it
   * would leave the leader unable to answer a review.
   */
...
describe("what the main agent asks a person before doing", () => {
  it("holds nothing at all", async () => {
    // A pull request is not worth stopping for: it is the point of the work, it
    // lands on a branch, and it is reviewable after the fact. So nothing here
    // waits on a person — pinned, because a rule added by accident stops a round
    // that should not wait, and only this notices.
...
const { freshStub: freshWorkspace } = makeDoHelpers<LeaderWorkspaceDO>(
  env.LEADER_WORKSPACE
);
...
describe.each([
  ...
  it("fails with a wiring sentence when no workspace reached it", async () => {
    ...
    const outcome = await runInDurableObject(stub, (instance: LeaderSubagent) =>
...
describe("a checkout with nothing to install", () => {
  it("is not mistaken for an empty workspace", async () => {
    ...
    const workspace = env.LEADER_WORKSPACE.get(
      env.LEADER_WORKSPACE.idFromName(name)
    );
...
describe("waiting for an interrupted session to unwind", () => {
  ...
  it("does not wait when the facet is holding no session", async () => {
    ...
    const interrupted = await runInDurableObject(
      stub,
      (instance: LeaderSubagent) => instance.abortRun()
    );
...
describe("a chunk replaced by a retry of itself", () => {
  it("has nothing to give back when no session is draining", async () => {
    ...
    await expect(
      runInDurableObject(stub, (instance: LeaderSubagent) =>
        instance.yieldRun()
      )
...
describe("stopping a session nothing is draining", () => {
  it("does nothing for a facet that never started one", async () => {
    ...
    await runInDurableObject(stub, (instance: LeaderSubagent) =>
  ...
  it("reaches for the session it recorded, and never throws for it", async () => {
    ...
    await runInDurableObject(stub, async (instance: LeaderSubagent, state) => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "[leader] could not stop the session on teardown",
      expect.anything()
    );
  });
});
...
describe("the branch a writing session is told about", () => {
  it("names the submodules, and says to commit inside each one", () => {
    const brief = sessionBrief(request(), undefined, {
      branch: "leader/task-1/1",
      submodules: ["core", "starter"],
      continues: false
    });

    expect(brief).toContain("You are on `leader/task-1/1`");
...
describe("the note on what a writing session kept", () => {
  ...
  it("says there is nothing to review when nothing was committed", () => {
    ...
    expect(note).toMatch(/No commits were made on `leader\/task-1\/1`/);
  });
 ...
 * so only an assertion catches it. `CLAUDE_CODE_SESSION` in `src/harness.ts`
 ...
 * `@/agents/leader/claude-code.ts`. What only this side can get wrong is
 ...
 * the image it would also reach the other harness's container, whose egress is
 * `direct`, and be presented to GitHub as a credential by anything that reads
 * `GH_TOKEN`.
 */
...
/**
 * The parent's pair is `MODEL`'s, inverted, and the inversion is the whole of
 * what this harness changes about it. Stated as a relationship rather than two
 * model ids so that moving the family stays a one-line edit, and asserted by
 * swapping the pair back: anything else that drifts apart — including a field
 * added to `MODEL` later — fails here.
 */
describe("the parent's model pair", () => {
  it("is the shared pair, inverted", () => {
    const parent = LEADER_CONFIG.model!;

    expect(parent.chatModelId).toBe(MODEL.fallbackChatModelId);
    expect(parent.fallbackChatModelId).toBe(MODEL.chatModelId);
    expect({
      ...parent,
      chatModelId: MODEL.chatModelId,
      fallbackChatModelId: MODEL.fallbackChatModelId
    }).toEqual(MODEL);
  });
});
