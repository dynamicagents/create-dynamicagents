...
import { LeaderSubagent } from "./subagent";

/** This agent's log prefix and workspace label. */
const LABEL = "leader";

/**
 * The leader, with subtasks on Claude Code.
 *
 ...
 * overrides below are all lifecycle, not inference.
 */
export class LeaderAgent extends RoundAgentBase<Env> {
   ...
   * subagent's list is one entry, because a Claude Code session brings its own
   * tools.
   */
  ...
  async reclaimIdleWorkspaces(): Promise<void> {
    ...
    const binding = this.env.LEADER_WORKSPACE;
   ...
   * workspace.
   *
   * The reasoning is on `discardWorkingTree`. It matters more under Claude Code
   * than on core's own loop: a cancelled session is stopped mid-turn with `SIGTERM`
   ...
   * arbitrary half-reset state. `LeaderSubagent.abortRun` therefore does
  ...
  protected override async onTaskCanceled(taskId: string): Promise<void> {
    ...
    await discardWorkingTree({
      binding: this.env.LEADER_WORKSPACE,
