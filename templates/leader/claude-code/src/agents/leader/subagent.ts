...
/**
 * The leader's subagent facet — a Claude Code session, not a model loop.
 *
 ...
 * {@link LeaderSubagent.abortExecution} needs to stop it with no drain in
 * hand.
 */
...
export async function settleDrain(
  ...
  console.warn(
    "[leader] the session did not unwind within the settle window; " +
...
/** What {@link LeaderSubagent} reports a session from. */
type SessionEnd = {
...
export class LeaderSubagent extends RecipeSubagentHost<Env> {
  protected agentConfig(): CoreConfigOverrides {
    return { ...LEADER_CONFIG, agentName: leader.tenant };
  }
  ...
  override async executeChunk(
    ...
    const stub = this.env.LEADER_WORKSPACE.get(
      this.env.LEADER_WORKSPACE.idFromName(name)
    );
  ...
  async #noteRateLimit(
    ...
    try {
      ...
      console.warn("[leader] could not record the bucket reading", {
        err: String(err)
      });
    }
  }
  ...
  override async abortRun(): Promise<boolean> {
    ...
    try {
      const stub = this.env.LEADER_WORKSPACE.get(
        this.env.LEADER_WORKSPACE.idFromName(inflight.name)
      );
      using workspace = await openWorkspace(stub);
      ...
      // not coming.
      console.warn("[leader] could not stop the session", {
        err: String(err)
      });
  ...
  override async abortExecution(toolFamilies: string[]): Promise<void> {
    ...
    try {
      const stub = this.env.LEADER_WORKSPACE.get(
        this.env.LEADER_WORKSPACE.idFromName(session.name)
      );
      using workspace = await openWorkspace(stub);
      await this.#session.stop(
        workspace.runtime as SessionRuntime,
      ...
      console.warn("[leader] could not stop the session on teardown", {
        err: String(err)
      });
    }
  }
  ...
  async #submodules(workspace: string, dir: string): Promise<string[]> {
    try {
      ...
      console.warn("[leader] could not read the checkout's submodules", {
        err: String(err)
      });
      return [];
    }
  ...
  async #recordStarts(
    ...
    try {
      ...
      console.warn("[leader] could not read where the session starts", {
        err: String(err)
      });
    }
  ...
  async #uncommitted(
    ...
    try {
      ...
      console.warn("[leader] could not read what is uncommitted", {
        err: String(err)
      });
      return [];
    }
  ...
  async #discard(
    ...
    try {
      ...
      if (!done.success) {
        console.warn("[leader] the uncommitted work was not all deleted", {
          stderr: done.stderr.trim().slice(0, 500)
        });
      }
      return { files: dirty, failed: !done.success };
    } catch (err) {
      console.warn("[leader] could not delete the uncommitted work", {
        err: String(err)
      });
  ...
  async #commits(
    ...
    try {
      ...
      console.warn("[leader] could not count the session's commits", {
        err: String(err)
      });
