 ...
 *
 * ## Two lists, and the split is sharp
 *
 * The parent delegates because a parent that reads files and runs builds
 * accumulates a session nobody can keep warm. A second reason lands on top of
 * that one: **the subagent is not running core's loop at
...
export function container(env: Env, name: () => string) {
  return workspaceContainer(env.LEADER_WORKSPACE, name);
}
...
export const parentPlugins = (host: PluginHost<Env>): AgentPlugin[] => {
  ...
  const binding = host.env.LEADER_WORKSPACE;
  /**
  ...
  return [
    ...
    /**
     * Episodic memory, which the leader on core's own loop does without.
     *
     * It earns its place under Claude Code for a reason specific to it: a session is
