 ...
 * gateway**. The Dockerfile is shared with the other harness, whose workspace
 * egresses `direct`: there nothing strips the header, and anything reading `GH_TOKEN` —
...
export function claudeCodeConfig(
  ...
  return {
     ...
     * other harness's container runs `mode: "direct"` with open egress, so a
     * restriction here would be a boundary only this harness had.
