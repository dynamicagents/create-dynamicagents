/**
 * The leader's soul — its frozen identity and operating rules.
 *
 * The leader delegates every edit and owns the git history. What it delegates
 * to shapes everything below: a `claude-code` subtask is a whole Claude Code
 * session — its own loop, its own tools, its own context
 ...
 *
 * The rules that follow from that are the ones that matter most below: clone
 * before delegating, brief for a whole change rather than a step, and review work that
...
export const SOUL: string[] = [
  ...
  // The review step, which is the parent's entire technical contribution. A
  // Claude Code session is autonomous for tens of minutes and reports a summary
  // of its own work, so the report is the least of what to read.
