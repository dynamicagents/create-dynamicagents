 ...
 *
 * Everything a workspace does is in `@dynamicagents/plugins/computer`. Two
 * things are this agent's own, and both exist so that the container
...
export class LeaderWorkspaceDO extends WorkspaceObjectBase {
  ...
  protected workspaceConfig(): WorkspaceObjectConfig {
    return {
      ...
      // Named rather than read because `workspaceConfig()` is reachable over
      // RPC — the base class carries the reasoning on `tokenBinding`.
  ...
  async claudeNoteRateLimit(info: RateLimitInfo): Promise<void> {
    ...
    console.warn(
      "[leader-workspace] retiring a credential on the " +
