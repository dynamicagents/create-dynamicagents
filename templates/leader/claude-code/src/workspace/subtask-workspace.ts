...
function assertSubmodulePath(path: string): void {
  if (
    ...
    throw new Error(
      `leader: refusing a submodule path that could leave the checkout: ${JSON.stringify(path)}`
    );
  }
}
...
export async function readSubmodules(
  ...
  if (!listed.success && listed.stderr.trim()) {
    throw new Error(
      `leader: could not read .gitmodules in ${dir}: ${listed.stderr.trim()}`
    );
  }
...
export function submoduleCloneUrl(sub: Submodule, parentUrl: string): string {
  ...
  if (parsed?.protocol !== "https:" || parsed.hostname !== host) {
    throw new Error(
      `leader: the submodule at ${sub.path} is cloned from ${sub.url}, and ` +
...
EOF`,
  ...
  if (!found.success) {
    throw new Error(
      `leader: could not read which submodules are cloned: ${found.stderr || found.stdout}`
    );
  }
...
export function subtaskWorkspaces(config: {
  ...
  async function prepare(
    ...
    if (!(await stub.checkoutDir())) {
      ...
      if (!/^\/[^/]+\/./.test(checkout.dir)) {
        throw new Error(
          `leader: refusing to clear a checkout path that is not under a workspace directory: ${checkout.dir}`
        );
      }
      ...
      if (!cleared.success) {
        throw new Error(
          `leader: could not clear an unfinished clone in a worktree: ${cleared.stderr || cleared.stdout}`
        );
      }
      ...
      if (!cloned.ok) {
        ...
        throw new Error(
          `leader: could not clone ${checkout.url} at ${checkout.branch} into a worktree: ${cloned.message}`
        );
      }
    ...
    const fetch = async (url: string, dir: string, what: string) => {
      ...
      if (!fetched.ok) {
        throw new Error(
          `leader: could not fetch ${what} in a worktree: ${fetched.message}`
        );
      }
    };
    const place = async (
      ...
      if (!placed.success) {
        throw new Error(
          `leader: could not put a worktree on ${worktree.branch}: ${placed.stderr || placed.stdout}`
        );
      }
    ...
    for (const sub of submodules) {
      ...
      if (!present.has(sub.path)) {
        ...
        if (!cloned.ok) {
          throw new Error(
            `leader: could not clone the submodule at ${sub.path}${branch ? ` on ${branch}` : ""}: ${cloned.message}`
          );
        }
      }
    ...
    if (
      ...
      throw new Error(
        `leader: no worktree holds ${worktree.branch}, and it is on the remote ` +
  ...
  return {
    async resolve(ctx): Promise<string> {
      ...
      if (!checkout) {
        throw new Error(
          "leader: there is no recorded checkout for a writing subtask to " +
      ...
      if (ctx.continue !== undefined && !isBranchName(ctx.continue)) {
        throw new Error(
          `leader: \`continue\` names ${JSON.stringify(ctx.continue)}, which is ` +
    ...
    async abort(ctx): Promise<void> {
       ...
       * facet's own cancellation waits that out — `LeaderSubagent.abortRun`
