...
import { HARNESS_TEST_ENV } from "./test/support/harness-env.ts";
 ...
 * binding, and the Durable Objects with their SQLite migration) so this config
...
// Never real: nothing in the suite reaches GitHub — the repo tools are tested
// against an injected `exec`. It exists only so `secrets.required` is satisfied
// and the pool stops warning.
//
// No model credential appears here because this Worker holds none for its own
// loop: it reaches Workers AI through the `AI` binding, which the platform
// authenticates.
//
...
// `secrets.required` while exercising the same `|| "da-leader"` fallback a
// real deploy takes when an operator leaves them unset.
process.env.GITHUB_NAME ??= "";
process.env.GITHUB_EMAIL ??= "";
// Whatever the subtask harness adds to `secrets.required` — one line per name
// there, since that list is what the generated `Env` types as a definite
// string.
for (const [name, value] of Object.entries(HARNESS_TEST_ENV)) {
  ...
  plugins: [
    cloudflareTest({
      ...
      miniflare: {
        ...
        // `ctx.exports.LeaderSubagent` is not facet-compatible and
        // `subAgent()` throws. See "Notes for testing" in
        // node_modules/agents/docs/sub-agents.md.
        durableObjects: {
          LEADER_SUBAGENT: {
            className: "LeaderSubagent",
            useSQLite: true
          }
        }
      }
