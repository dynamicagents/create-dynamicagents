 ...
 * Kept next to the handler rather than in `round-policy.ts` because they are
 * about this Worker's credentials, not about how a round ends.
 *
...
/** The leader's task workflow: core's orchestration, its own binding. */
export class LeaderWorkflow extends WorkflowEntrypoint<Env, HandleTaskParams> {
  /**
  ...
  async run(
    ...
    return await runHandleTask(event.payload, step, {
      resolveAgent: (identity) => leader.resolveAgent(this.env, identity),
      config: resolveConfig(LEADER_CONFIG),
      policy: roundPolicy,
      ...
      // Names this agent in the abandoned-task log line.
      label: "leader"
    });
  }
}
