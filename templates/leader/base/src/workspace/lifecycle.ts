 ...
 *
 * Here rather than in `agent.ts` because either harness's agent owes them, and
 * both are the kind of code that is subtly wrong in a copy:
 * one is a cleanup that must not throw away the expensive thing, the other is a
 * sweep whose whole job is to decide nothing.
 */

/** The workspace namespace, typed on the shared base class. */
