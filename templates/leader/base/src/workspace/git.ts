 ...
 *
 * Shared by both harnesses, and it has to be: the credential rule below is the
 * one thing about git in this repository that must not be re-implemented.
 *
...
export function workspaceGit(config: {
  /**
   * The workspace namespace this call belongs to.
   *
   * Typed on the shared base rather than on `LeaderWorkspaceDO`: the RPCs below
   * are declared there, and naming the subclass would tie this function to one
   * harness's workspace for no reason a caller could act on.
   */
