 ...
 *
 * Only the Claude Code harness has writing subtasks in worktrees; its
 * `worktrees.ts` and `subtask-workspace.ts` sit beside this file there. This one
 * is shared because `./lifecycle.ts` has to recognise a worktree's sentinel on
 * either harness.
 *
...
export function subtaskBranch(ctx: {
  ...
  return `leader/${ctx.taskId}/${ctx.subtaskId}`;
}
...
export function claim(
  ...
  if (holder?.live) {
    throw new Error(
      `leader: ${ctx.continue} is being worked on by subtask ${holder.live.subtaskId} ` +
