...
describe("a round that may wait", () => {
  ...
  it("describes the wait only to a round that has one", () => {
    ...
    // A round that has spent its allowance. A contract naming a call the round
    // is not handed is a contract inviting a rejected call.
  ...
  it("supplies the note core refuses to invent", () => {
    ...
    // and the leader does enable them — so a policy missing this fails at DO start
