...
describe("addressing a worktree", () => {
  ...
  it("derives a branch name, and lets `continue` name any branch git accepts", () => {
    ...
    expect(branch).toBe("leader/task-a/3");
...
describe("claiming a worktree", () => {
  ...
  it("reuses the free worktree used longest ago, and names what it held", () => {
    ...
    expect(claimed.previous).toBe("leader/t/2");
  ...
  it("continues a branch in the worktree that holds it", () => {
    ...
    const claimed = claim(pool, REPO, { ...ctx, continue: "leader/t/1" }, 10);
  ...
  it("refuses to continue a branch a session is still working on", () => {
    ...
    expect(() =>
      claim(pool, REPO, { ...ctx, continue: "leader/t/1" }, 10)
    ).toThrow(/leader\/t\/1 is being worked on by subtask 1/);
  });
...
describe("the pool in the parent's SQLite", () => {
  it("keeps rows per repository and slot, and finds them across repositories", async () => {
    const stub = env.LEADER_WORKSPACE.get(
      env.LEADER_WORKSPACE.idFromName("pool-spec")
    );
